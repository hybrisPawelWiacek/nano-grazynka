import { EntityContextBuilder } from '../../../backend/src/application/services/EntityContextBuilder';
import type { IEntityRepository } from '../../../backend/src/domain/repositories/IEntityRepository';
import type { IProjectRepository } from '../../../backend/src/domain/repositories/IProjectRepository';
import type { Entity } from '../../../backend/src/domain/models/Entity';
...
describe('EntityContextBuilder', () => {
  ...
  beforeEach(() => {
    ...
    mockProjectRepo = {
      ...
      getActiveProjectForUser: jest.fn()
    };
  ...
  describe('compressForGPT4o', () => {
    ...
    it('should limit individual type lists to 5 entities each', async () => {
      const mockEntities: Entity[] = [];
      
      // Create 10 technical entities
      for (let i = 1; i <= 10; i++) {
        mockEntities.push({
          id: `tech-${i}`,
          userId: 'user1',
      ...
      // Technical field should have only 5 entities
      const technicalEntities = context.technical.split(', ');
      expect(technicalEntities.length).toBe(5);
      expect(technicalEntities).toEqual([
        'TechValue1',
        'TechValue2',
        'TechValue3',
        'TechValue4',
  ...
  describe('expandForGemini', () => {
    ...
    it('should include aliases for technical terms in Gemini context', async () => {
      const mockEntities: Entity[] = [
        {
          id: '1',
          userId: 'user1',
      ...
      expect(context.technical).toBe('RLHF (aka: Reinforcement Learning from Human Feedback, RLHF), API');
    });
    ...
    it('should handle empty sections in detailed context', async () => {
      const mockEntities: Entity[] = [
        {
          id: '1',
          userId: 'user1',
      ...
      expect(context.detailed).toBeDefined();
      expect(context.detailed).toContain('Technical Terms and Concepts:');
      expect(context.detailed).not.toContain('Team Members and People:');
      expect(context.detailed).not.toContain('Companies and Organizations:');
      expect(context.detailed).not.toContain('Products and Services:');
    });
  });

  describe('groupByType', () => {
    it('should correctly group entities by type', async () => {
      const mockEntities: Entity[] = [
        {
          id: '1',
          userId: 'user1',
      ...
      // Check that entities are properly grouped in the compressed output
      // Technical should come first, then person, then company
      const compressed = context.compressed.split(', ');
      expect(compressed[0]).toBe('Tech1');
      expect(compressed[1]).toBe('Person1');
      expect(compressed[2]).toBe('Person2');
      expect(compressed[3]).toBe('Company1');
    });
  });
});